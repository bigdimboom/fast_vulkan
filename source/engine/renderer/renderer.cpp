...
Renderer::Renderer(window::VKWindow& window, std::shared_ptr<camera::FreeCamera> cam)
{
...
void Renderer::destroyAPI()
{
	...
	// destroy debug if it's enabled
	if (mDebug)
	{
		mDldy.vkDestroyDebugReportCallbackEXT(mInstance, mDebug, nullptr);
	}
...
void Renderer::initializeAPI(window::VKWindow& window)
{
	/**
	* Initialize the Vulkan API by creating its various API entry points:
	*/

	auto wantedExtensions = window.vkInstanceExtensions();
	...
	vk::InstanceCreateInfo info(
		...
		layers.data(),
		static_cast<uint32_t>(wantedExtensions.size()),
		wantedExtensions.data()
	);

	mInstance = vk::createInstance(info);

	if (app::SystemMgr::instance().settings().is_debug_enabled)
	{
		mDldy.init(mInstance);
		...
		auto creat_func = mDldy.vkCreateDebugReportCallbackEXT;

		if (creat_func)
		{
			creat_func(mInstance, &debug_report_ci, nullptr, &handle);
			mDebug = vk::DebugReportCallbackEXT(handle);
		}
		else
		{
	...
	//Swapchain
	auto pixelRez = window.pixelrez();
	setupSwapchain(pixelRez.x, pixelRez.y);
	mCurrentBuffer = 0;
...
void Renderer::setupSwapchain(unsigned width, unsigned height)
{
	...
	uint32_t backbufferCount = glm::clamp((uint32_t)app::SystemMgr::instance().settings().back_buffer_count,
		2u, surfaceCapabilities.maxImageCount);
	...
	mSurfaceSize = vk::Extent2D(glm::clamp(swapchainSize.width, 1U, 8192U), glm::clamp(swapchainSize.height, 1U, 8192U));
...
void Renderer::initializeResources()
{
	...
	*/
	...
	if (mCamera)
	{
		uboVS.projectionMatrix = mCamera->proj();
	}
	else
	{
		uboVS.projectionMatrix = glm::perspective(glm::radians(45.0f), (float)mViewport.width / (float)mViewport.height, 0.01f, 1024.0f);
	}
	...
	std::vector<char> vertShaderCode =
		readFile(app::SystemMgr::instance().settings().shader_dir + "triangle.vert.spv");
	std::vector<char> fragShaderCode =
		readFile(app::SystemMgr::instance().settings().shader_dir + "triangle.frag.spv");

	//auto vs_path = "./shaders/triangle.vert";
	//auto fs_path = "./shaders/triangle.frag";

	//std::ifstream vert_ifs(vs_path);
	//std::ifstream frag_ifs(fs_path);
	//std::string vert_glsl((std::istreambuf_iterator<char>(vert_ifs)), (std::istreambuf_iterator<char>()));
	//std::string frag_glsl((std::istreambuf_iterator<char>(frag_ifs)), (std::istreambuf_iterator<char>()));

	//shaderc::CompileOptions cOptions;
	//cOptions.SetSourceLanguage(shaderc_source_language::shaderc_source_language_glsl);
	////cOptions.SetOptimizationLevel(shaderc_optimization_level::shaderc_optimization_level_size);

	//shaderc::Compiler shaderCompiler;
	//auto vs = shaderCompiler.CompileGlslToSpvAssembly(vert_glsl, shaderc_shader_kind::shaderc_vertex_shader, vs_path, cOptions);
	//auto fs = shaderCompiler.CompileGlslToSpvAssembly(frag_glsl, shaderc_shader_kind::shaderc_fragment_shader, fs_path, cOptions);

	//std::string vsAssembly(vs.cbegin(), vs.cend());
	//std::string fsAssembly(fs.cbegin(), fs.cend());
...
void Renderer::render()
{
	...
	uboVS.modelMatrix = glm::rotate(glm::mat4(1.0f), mElapsedTime, glm::vec3(0.0f, 1.0f, 0.0));

	if (mCamera)
	{
		uboVS.viewMatrix = mCamera->view();
	}
